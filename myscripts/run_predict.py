...
web_base = '/var/www/html/outputs/results/'
...
class RUN:
    ...
    def alphafold2(self, faFile):
        ...
        cmd = ". /home/nong/anaconda3/etc/profile.d/conda.sh; conda activate alphafold; "
        cmd += f"python3 /training/nong/protein/apps/alphafold/docker/run_docker.py --fasta_paths={faFile} --max_template_date=2020-05-14 --preset=casp14 2>{log}; "
        print("running alphafold 2 =============================>>>>>>>>>>>>>>>>>>>>>>> ")
        run = subprocess.run(cmd, shell=True)
        print("alphafold returncode: >>>>>>>>>>>>>>> ", run.returncode)

        if run.returncode == 0:
            os.system(f'cd {res_root}; tar -czvf {pre}_alphafold.tar.gz {pre}/rank*pdb')
            os.system(f'mkdir -p {web_dir}/{pre}')
            os.system(f'cp {res_root}/{pre}_alphafold.tar.gz {web_dir}')
            os.system(f"cp -r {res_dir}/rank*pdb  {web_dir}/{pre}")
    ...
    def roseTTAFold(self, faFile, mode="pyrosetta"):
        ...
        cmd = ". /home/nong/anaconda3/etc/profile.d/conda.sh; conda activate RoseTTAFold;"
        pre = os.path.basename(faFile).split('.fa')[0]
        ...
        if run.returncode == 0:
            ...
            if bins[mode]=='run_pyrosetta_ver.sh':
                os.system(cmd_res)
                os.system(f'cd {res_root}; tar -czvf {pre}_roseTTAFold.tar.gz {pre}/model')
                os.system(f'cp -r {res_dir}/model {web_dir}/{pre}/')
            else:
                os.system(f'cd {res_root}; tar -czvf {pre}_roseTTAFold.tar.gz {pre}/t000_.e2e.pdb') # end to end
    ...
    def run(self, faFile):
        ...
        if obj.running_date is not None:
            ...
            if "AlphaFold 2" in obj.tools:
                print('users select alphafold!')
                self.alphafold2(faFile)
                if self.alphafold2_code == 0:
                    status[0] = '0'
                    print(self.pre, "alphafold2 success! =================================>")
            if 'RoseTTAFold' in obj.tools:
                print('user select roseTTAFold!')
                self.roseTTAFold(faFile,  roseMode )
                if self.roseTTAFold_code == 0:
                    status[1] = '0'
                    print(self.pre, "RoseTTAFold success! =================================>")
            obj.run_status = ','.join(status)
            obj.completed_date = timezone.now()
            obj.save()

    def watchdog(self, mypath='./protein/static/uploads'):
