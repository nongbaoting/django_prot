...
class SPalign:
    ...
    def __init__(self, path_1, path_2, tempDir):
        self.path_1 = path_1
        self.path_2 = path_2
    ...
    def align(self):
        cmd = 'cd %s;SP-align.intel %s %s >Spalgin.txt' % (
            self.tempDir, self.path_1, self.path_2)
        run_cmd(cmd)
        self.logFi = "Spalgin.txt"
    ...
    def rotate(self, ):
        outPDB = os.path.join(
            self.tempDir, self.content["input_pdb"] + "_out1.pdb")
        self.content['out1_pdb'] = outPDB
...
def run_spalign(path1, path2, tempDir):
    spalign = SPalign(path1, path2, tempDir)
    spalign.align()
